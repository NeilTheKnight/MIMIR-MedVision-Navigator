...
from PyQt5.QtWidgets import (
    ...
    QGridLayout, QLabel, QListWidget, QTextBrowser, QFrame, QListWidgetItem, QMenu
)
...
class CNNClassificationThread(QThread):
    ...
    def run(self):
        try:
            ...
            model = gr.load_trained_model(gr.MODEL_PATH)
            out_path = gr.write_report_for_folder(model, self.folder_path, gr.OUTPUT_DIR)
            content = ""
            if out_path and os.path.exists(out_path):
                with open(out_path, "r", encoding="utf-8") as f:
                    content = f.read()
            else:
                content = f"未生成报告，请检查文件夹：{self.folder_path}"
...
class MedicalViewer(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("脑肿瘤分类界面-Brain Tumor Recognition ")
    ...
    def _init_ui(self):
        ...
        title_label = QLabel("脑肿瘤分类界面-Brain Tumor Recognition ")
        ...
        self.ct_image_list.itemClicked.connect(self._on_cnn_folder_selected)
        ...
        # --- 4. 右侧列（1/5宽度，AI分析 + 仪表盘）---
        right_panel = self._create_panel()
        right_layout = QVBoxLayout(right_panel)
        self.main_layout.addWidget(right_panel, 1, 2, 4, 1)

        # 4.1 仅保留AI分析面板，占满整个右侧
        analysis_title = QLabel("Image Analysis & Statistics")
        analysis_title.setStyleSheet("font-size: 11pt; font-weight: bold; margin-bottom: 8px;")
        self.analysis_text_browser = QTextBrowser()
        self.analysis_text_browser.setText("Waiting for AI analysis...")
        right_layout.addWidget(analysis_title)
        right_layout.addWidget(self.analysis_text_browser)
        # 让分析文本框在右侧自上而下充满
    ...
    def _load_initial_data(self):
        # 加载 CNN 文件夹列表；默认选择“1”或首个文件夹
        self._load_cnn_folder_list()
        if hasattr(self, 'ct_image_list') and self.ct_image_list.count() > 0:
            default_row = 0
            for i in range(self.ct_image_list.count()):
                if self.ct_image_list.item(i).text().strip() == "1":
                    default_row = i
                    break
            self.ct_image_list.setCurrentRow(default_row)
            item = self.ct_image_list.item(default_row)
            self._on_cnn_folder_selected(item)
        else:
    ...
    def _load_cnn_folder_list(self):
        """读取 config.py 中配置的 cnn_images 下的子文件夹，作为可选项"""
        self.ct_image_list.clear()
        cnn_images_dir = PATHS["cnn_images"]
        if not os.path.isdir(cnn_images_dir):
            return
        subdirs = [d for d in sorted(os.listdir(cnn_images_dir)) if os.path.isdir(os.path.join(cnn_images_dir, d))]
        for d in subdirs:
            folder_path = os.path.join(cnn_images_dir, d)
            icon = QIcon()
            try:
                imgs = [f for f in os.listdir(folder_path) if f.lower().endswith((".jpg", ".png", ".jpeg", ".bmp"))]
                if imgs:
                    thumb_path = os.path.join(folder_path, sorted(imgs)[0])
                    pix = QPixmap(thumb_path).scaled(160, 100, Qt.KeepAspectRatio, Qt.SmoothTransformation)
                    icon = QIcon(pix)
            except Exception:
                pass
            item = QListWidgetItem(icon, d)
            item.setData(Qt.UserRole, folder_path)
            item.setSizeHint(QSize(180, 110))
            self.ct_image_list.addItem(item)

    def _on_cnn_folder_selected(self, item):
        folder_path = item.data(Qt.UserRole)
        if not folder_path or not os.path.isdir(folder_path):
            return
        self._load_images_from_folder(folder_path)
        self._start_cnn_classification(folder_path)

    def _load_images_from_folder(self, folder_path: str):
        exts = (".jpg", ".jpeg", ".png", ".bmp")
        files = [f for f in sorted(os.listdir(folder_path)) if f.lower().endswith(exts)]
        selected = files[:4]
        keys = ["A", "B", "C", "D"]
    ...
    def _start_cnn_classification(self, folder_path: str):
        if hasattr(self, 'cnn_thread') and self.cnn_thread.isRunning():
            try:
                self.cnn_thread.quit()
                self.cnn_thread.wait()
            except Exception:
                pass
        self.analysis_text_browser.setText("正在生成分类报告，请稍候...")
        self.cnn_thread = CNNClassificationThread(folder_path)
        self.cnn_thread.report_ready.connect(self._update_analysis_text)
        self.cnn_thread.error_occurred.connect(self._show_error)
    ...
    def _update_analysis_text(self, text):
        # 保留模拟文本接口，但实际将通过报告文件更新
        self.analysis_text_browser.setText(text)

    def _load_report(self, img_basename):
        # 从 config.py 中配置的路径读取详细报告
        info_dir = PATHS["inference_info"]
        info_file = os.path.join(info_dir, f"{img_basename}_info.txt")
        if os.path.exists(info_file):
            try:
                with open(info_file, "r", encoding="utf-8") as f:
                    self.analysis_text_browser.setText(f.read())
            except Exception as e:
                self.analysis_text_browser.setText(f"错误：无法读取报告文件\nError reading report file: {str(e)}")
        else:
            self.analysis_text_browser.setText(
                f"报告文件不存在\nReport file not found:\n{info_file}\n\n图像可能正在处理中...\nImage may still be processing..."
            )

    def _show_error(self, error_message):
        self.analysis_text_browser.setText(error_message)

    def _load_ct_image_list(self):
        """读取 config.py 中配置的 test_images，按文件名排序并加载为可选择列表"""
        self.ct_image_list.clear()
    ...
    def _process_selected_image(self, image_path):
        ...
        if TumorImageProcessorThread is not None:
            self.processor_thread = TumorImageProcessorThread(selected_image_path=image_path)
            self.processor_thread.images_processed.connect(self._update_images)
            try:
            ...
            try:
                ...
                pass
            self.processor_thread.start()
            self.analysis_text_browser.setText("正在处理图像...\nProcessing image...")
        else:
            # 无真实推理时，回退到模拟线程
            self.processor_thread = ImageProcessorThread()
            self.processor_thread.images_processed.connect(self._update_images)
